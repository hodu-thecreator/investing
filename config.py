...
class Config:
    ...
    # ── 동적 현금 목표 — 거시 위험점수(0~10+) 기반 사다리 (2026.6 신설) ──
    # 평시 헌법 5조 20%가 기본값. daily_report.calc_macro_risk_score()가 집계한
    # 위험 신호(버핏지수/신용스프레드/금리차/공포탐욕/VIX/AAII/브레드스)가 쌓일수록
    # 조정 대비 현금 목표를 단계적으로 올림 (최대 50%).
    # "오른쪽 어깨"(이미 꺾임 확인 후)부터 본격 비축하는 보수적 사다리 —
    # 초반엔 헌법값 가깝게 유지하고, 신호가 누적될수록 단계를 가속.
    CASH_TARGET_LADDER: list[tuple[int, float]] = [
        (3, 0.25),
        (5, 0.35),
        (7, 0.45),
        (9, 0.50),
    ]
    # 현금 목표가 위 사다리로 올라가 갭이 생겼을 때, 리포트 1회당 익절 가능한
    # 최대 비중(%p, 총자산 기준) — 헌법 7조 "팔지 않기가 기본"과 균형.
    # 갭은 한 번에 메우지 않고 여러 리포트에 걸쳐 점진적으로 해소.
    CASH_TRIM_CAP_PCT_PER_REPORT = 0.05
    # 종목 1개를 한 번에 트림할 수 있는 최대 비율(그 종목 보유 평가액 대비) —
    # 포지션 의미는 유지한 채 일부만 정리.
    CASH_TRIM_MAX_PER_TICKER_PCT = 0.15
